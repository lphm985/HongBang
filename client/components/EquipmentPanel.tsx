...
import type { Player, Equipment, EquipmentBonus, EquipmentSlot } from '../types';
import { EQUIPMENT, formatNumber } from '../constants';
...
}> = ({ slot, itemId, onItemClick }) => {
    const item = itemId ? EQUIPMENT.find(eq => eq.id === itemId) : null;
...
interface EquipmentPanelProps {
  ...
  onListItem: (itemId: string, price: number) => void; // NEW: Prop for listing an item
}
...
const EquipmentPanel: React.FC<EquipmentPanelProps> = ({ player, onEquipItem, onListItem }) => {
    const inventoryItems = player.inventory.map(id => EQUIPMENT.find(eq => eq.id === id)).filter((item): item is Equipment => !!item);
    const [itemToSell, setItemToSell] = useState<Equipment | null>(null);
    
    const handleConfirmSell = (price: number) => {
        if (itemToSell) {
            onListItem(itemToSell.id, price);
            setItemToSell(null);
        }
    };
    ...
    return (
        <div className="flex flex-col h-full space-y-4">
            ...
            <div className="space-y-2">
                <EquipmentSlotDisplay slot="weapon" itemId={player.equipment.weapon} onItemClick={() => player.equipment.weapon && onEquipItem(player.equipment.weapon)} />
                <EquipmentSlotDisplay slot="armor" itemId={player.equipment.armor} onItemClick={() => player.equipment.armor && onEquipItem(player.equipment.armor)} />
                <EquipmentSlotDisplay slot="accessory" itemId={player.equipment.accessory} onItemClick={() => player.equipment.accessory && onEquipItem(player.equipment.accessory)} />
            </div>
            ...
            <div className="flex-grow bg-slate-900/50 p-3 rounded-lg border border-slate-700 flex flex-col min-h-0">
                ...
                <div className="overflow-y-auto pr-2 space-y-3">
                    {inventoryItems.length > 0 ? inventoryItems.map(item => (
                        <div key={item.id} className="bg-slate-800/60 p-3 rounded-lg">
