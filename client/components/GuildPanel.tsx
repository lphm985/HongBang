...
import { REALMS, GUILD_CREATION_COST, getGuildNextLevelExp, getGuildBonuses, formatNumber, getGuildMemberLimit } from '../constants';
...
const GuildPanel: React.FC<GuildPanelProps> = ({ player, token, onCreateGuild, onJoinGuild, onLeaveGuild, onContributeToGuild, onInspectPlayer, showConfirmation }) => {
                    ...
                    <div className="overflow-y-auto pr-2 space-y-2 flex-grow">
                        {guildDetails.members.map((member) => (
                            <div key={member.name} className="flex justify-between items-center bg-slate-800/60 p-2 rounded">
                                ...
                                <span className="text-xs text-slate-400">{REALMS[member.realmIndex]?.name || 'N/A'}</span>
                            </div>
                        ))}
                    </div>
                </div>
