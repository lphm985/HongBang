...
import type { Player, ExplorationLocation } from '../types';
import { BODY_STRENGTH_COST, EXPLORATION_LOCATIONS, REALMS, HERBS, formatNumber } from '../constants';
...
const BodyTemperingPanel: React.FC<BodyTemperingPanelProps> = ({ player, onTemperBody, onStartExploration, explorationStatus }) => {
            ...
            return (
              <div key={location.id} className="bg-slate-900/50 p-3 rounded-lg border border-slate-700">
                ...
                <p className="text-xs text-slate-500 mt-1">
                  Yêu cầu: Tu vi [{REALMS[location.requiredRealmIndex].name}], Thân thể [{location.requiredBodyStrength}]
                </p>
                <div className="mt-3">
                  <button
                    onClick={() => onStartExploration(location)}
                    disabled={!canExplore || explorationStatus !== null}
                    className="w-full px-4 py-2 text-sm font-bold rounded-lg shadow-md transition-all duration-300 ease-in-out bg-indigo-600 hover:bg-indigo-700 text-white focus:outline-none focus:ring-4 focus:ring-indigo-400/50 disabled:bg-slate-600 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {isExploringThis 
