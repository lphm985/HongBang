...
import type { Player, GameEvent, TrialZone, Reward } from '../types';
import { TRIAL_ZONES, REALMS, HERBS, formatNumber } from '../constants';
...
const formatReward = (reward: Reward): string | null => {
    switch (reward.type) {
        case 'qi': return `${formatNumber(reward.amount)} Linh Khí`;
        case 'herb':
            const herb = HERBS.find(h => h.id === reward.herbId);
            return `${herb?.name} x${reward.amount}`;
        default: return null;
    }
};

const TrialGroundsPanel: React.FC<TrialGroundsPanelProps> = ({ player, token, lastChallengeTime, onChallengeResult, addEvent }) => {
                    ...
                    <div key={zone.id} className="bg-slate-900/50 p-3 rounded-lg border border-slate-700">
                        ...
                        <p className="text-xs text-slate-400 mt-1 italic">{zone.description}</p>
                         <p className="text-xs text-slate-500 mt-2">
                            Phần thưởng: {zone.rewards.map(formatReward).filter(Boolean).join(', ')}
                        </p>
                        <p className="text-xs text-slate-500 mt-1">
                            Yêu cầu: Tu vi [{REALMS[zone.requiredRealmIndex].name}]
                        </p>
                        <div className="mt-3">
                            <button
