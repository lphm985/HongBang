...
import { RECIPES, PILLS, HERBS, REALMS, formatNumber } from '../constants';
...
const AlchemyPanel: React.FC<AlchemyPanelProps> = ({ player, onCraftPill, onUsePill }) => {
        ...
        <div className="flex flex-col space-y-4">
            ...
            <div className="bg-slate-900/50 p-3 rounded-lg border border-slate-700 flex-shrink-0">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-sm">
                 ...
                 {HERBS.length === 0 && <p className="text-xs text-slate-500">Túi nguyên liệu trống.</p>}
            </div>
                             ...
                             return (
                                <div key={recipe.id} className={`bg-slate-800/60 p-3 rounded-lg ${!meetsRealmReq ? 'opacity-50' : ''}`}>
                                    ...
                                    <div className="text-xs text-slate-500 mt-2 space-y-1">
                                         <p>Yêu cầu: {REALMS[recipe.requiredRealmIndex].name}</p>
