...
import type { Player, GuildWarState, GuildMember, GuildWarFightResult } from '../types';
import { REALMS } from '../constants'; // Import REALMS for member display
...
const GuildWarPanel: React.FC<GuildWarPanelProps> = ({ token, player }) => {
        ...
        <div className="space-y-4">
            ...
            {current_war.status === 'IN_PROGRESS' && (
                <>
                    ...
                    {my_match && <MatchView match={my_match} token={token} player={player} onAction={fetchState} fightResults={state.fight_results || []} isLeader={is_leader} />}
                </>
            )}
...
const BattlefieldResultsView = ({ roundNumber, fights, guild1Name, guild2Name }: { roundNumber: number, fights: GuildWarFightResult[], guild1Name: string, guild2Name: string }) => {
    ...
    // FIX: Corrected logic to determine winner and opponent from guild1's perspective.
    // This removes the dependency on the 'player' variable which was not in scope, fixing the error.
    const matchForModal = (fight: GuildWarFightResult): any => ({
        id: fight.id,
        won: fight.winner_player === fight.guild1_player,
        opponent: fight.guild2_player,
...
const MatchView = ({ match, token, player, onAction, fightResults, isLeader }: { match: GuildWarState['my_match'], token: string | null, player: Player, onAction: () => void, fightResults: GuildWarFightResult[], isLeader: boolean }) => {
    ...
    return (
        <div className="space-y-4">
            ...
            {match.status === 'PENDING_LINEUP' && (
                 <LineupSelectionView matchId={match.id} token={token} player={player} onLineupSubmitted={onAction} myLineupSubmitted={match.my_lineup_submitted} opponentLineupSubmitted={match.opponent_lineup_submitted} isLeader={isLeader} />
            )}
...
const LineupSelectionView = ({ matchId, token, player, onLineupSubmitted, myLineupSubmitted, opponentLineupSubmitted, isLeader }) => {
    ...
    const positions = { vanguard: 'Tiên Phong', mid: 'Trung Quân', commander: 'Chủ Soái' };
    ...
    return (
        <div className="bg-slate-900/50 p-4 rounded-lg border border-slate-700 space-y-4">
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                 {Object.keys(positions).map((pos: 'vanguard' | 'mid' | 'commander') => (
                             ...
                             {eligibleMembers.map(member => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <p className="text-xs text-slate-400">{REALMS[member.realmIndex]?.name || 'N/A'}</p>
