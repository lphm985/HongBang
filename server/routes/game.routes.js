    ...
    await performAction(req, res, async (conn, p, body, resRef) => {
        ...
        if (playerHealth > 0) { // WIN
            ...
            zone.rewards.forEach(reward => {
                ...
                // FIX: Updated reward logic for the new equipment system
                if (reward.type === 'equipment') {
                    if (!p.inventory.includes(reward.equipmentId)) {
                         p.inventory.push(reward.equipmentId);
            ...
            updates.inventory = JSON.stringify(p.inventory); // FIX: Save inventory
...
// FIX: Renamed from /equip-treasure to /equip-item and updated logic
router.post('/equip-item', authenticateToken, async (req, res) => {
    const { itemId } = req.body;
    await performAction(req, res, async (conn, p, body, resRef) => {
        const gameData = getGameData();
        const item = gameData.EQUIPMENT.find(t => t.id === itemId);
        if (!item) throw new Error('Trang bị không tồn tại.');
        if (!p.inventory.includes(itemId)) throw new Error('Bạn không sở hữu trang bị này.');
        
        const slot = item.slot;
        const currentEquippedId = p.equipment[slot];
        const newEquipment = { ...p.equipment };
        const newInventory = [...p.inventory];

        if (currentEquippedId === itemId) { // Unequip
            delete newEquipment[slot];
            resRef.log = { message: `Bạn đã tháo [${item.name}].`, type: 'info' };
        } else { // Equip
            // Move currently equipped item back to inventory if there is one
            if (currentEquippedId) {
                newInventory.push(currentEquippedId);
            }
            // Set new item
            ...
            resRef.log = { message: `Bạn đã trang bị [${item.name}].`, type: 'success' };
        }
...
router.post('/reset', authenticateToken, async (req, res) => {
     await performAction(req, res, async (conn, p, body, resRef) => {
        // FIX: Updated to reset new equipment fields
        const updates = {
            qi: 0, 
            realmIndex: 0, 
            bodyStrength: 0, 
