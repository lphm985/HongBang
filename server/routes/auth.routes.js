...
router.post('/register', async (req, res) => {
    ...
    try {
        ...
        // FIX: Updated INSERT statement to use the new equipment system schema
        await conn.query(
            "INSERT INTO players (name, password, karma, learnedTechniques, activeTechniqueId, lastChallengeTime, pills, herbs, spiritualRoot, inventory, equipment, honorPoints, enlightenmentPoints, unlockedInsights, purchasedHonorItems, explorationStatus, linh_thach, pvpBuff) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            [
                name,
                hashedPassword,
                0, // karma
                ...
                assignedRootId,
                JSON.stringify([]), // inventory (new)
                JSON.stringify({}), // equipment (new)
                0, // honorPoints
